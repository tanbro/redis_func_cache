...
- It cannot hit cache across different Python versions by default. Because:
  ...
  from redis_func_cache.scripts import LfuScripts
...
- The Redis keys generated by *Multiple* policies include a hash derived from Python bytecode, making them **incompatible across Python versions**.
  ...
  However, you can define a custom hasher that subclasses `hashing.Hasher`, in which you can implement your own hash function to support compatibility across Python versions.
