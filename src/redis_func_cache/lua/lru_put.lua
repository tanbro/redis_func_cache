...
local c = 0
-- Max members per HDEL/SPOP chunk: Lua unpack() overflows above ~8000 values
...
if redis.call('ZRANK', zset_key, hash) then
    ...
    if maxsize > 0 then
        ...
        if n >= 0 then
            ...
            -- Remove evicted keys from hash map (chunked: unpack has a stack limit) and update eviction count
