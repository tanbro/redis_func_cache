...
- 💔 **Breaking Changes:**
  ...
  - `RedisFuncCache.__init__`: the `client` parameter is renamed `redis_client`. `client=` still works but emits a `DeprecationWarning`.
  - `RedisFuncCache.get_client()` is renamed `get_redis_client()`. The old name still works but emits a `DeprecationWarning`; the deprecated `client` property now points to the new name as well.
...
- ✨ **New Features:**
  - `handler` option on `RedisFuncCache` (constructor) and on `decorate()` (per-function override): a single optional handler that wraps the four serialization boundaries — `before_serialize`, `after_serialize`, `before_deserialize`, `after_deserialize` — each with an `*_async` counterpart. Built for use cases the `serializer` pair cannot express, notably **offloading large values to object storage** (async I/O, per-invocation context, and per-boundary override). `HandlerProtocol` is a pure structural protocol — implementations need not inherit from it; implement the boundaries you support (sync, async, or both) and have unsupported ones `raise NotImplementedError`. `before_*` methods return `(handled, value)` — when `handled` is true the library skips everything remaining on that side, including the corresponding `after_*` method; `after_*` methods return the replacement value directly. Every method receives an immutable `HandlerContext` (`keys`, `hash_value`, `func`, `args`, `kwds`) whose `args`/`kwds` are the same excludes-filtered arguments used for cache-key computation. The execution paths call the handler methods statically, one to one; the asynchronous path calls **only** the `*_async` methods (no async-to-sync fallback, so blocking I/O can never be introduced onto the event loop implicitly). The library performs **no** runtime validation of handler behavior — the type annotations are the contract, and handler exceptions propagate with no library retry, degradation, or counting. `HandlerProtocol` and `HandlerContext` are exported from the package root. Design note: `docs/design/handler.md`.
...
- 📚 **Documentation:**
  - New design notes: "Handler System for `redis_func_cache`" (`docs/design/handler.md`), "Vacuuming Expired Per-Field Cache Entries" (`docs/design/field-ttl-vacuum.md`), and "Purging Cache Structures Without Blocking Redis" (`docs/design/purge.md`).
  - README: new *Handler* section under Advanced Usage, including the handler-vs-serializer decision guide.
