...
- ✨ **New Features:**
  - `make_hash_mixin(name, hash_config, base=None)` factory in `redis_func_cache.mixins.hash`: builds a hash mixin class from a `HashConfig` (any `hashlib` algorithm, any serializer/decoder callables, optional `use_bytecode`) instead of hand-writing a subclass — for combinations not covered by the 24 predefined classes (e.g. msgpack + `sha3_256`). Likewise `make_scripts_mixin(name, scripts, base=None)` in `redis_func_cache.mixins.scripts` builds a scripts mixin from a pair of Lua script file names. Both accept an optional custom base class and return a fresh class per call (no memoization: type checks should target `AbstractHashMixin` / `AbstractScriptsMixin`). The predefined mixin classes are unchanged, and generated classes with standard parts hash identically to their hand-written equivalents. Design note: `docs/design/hash-mixin-factory.md`.
