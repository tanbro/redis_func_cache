...
def test_update_ttl_override_in_decorate():
    """测试在decorate方法中覆盖update_ttl参数"""
    for cache in CACHES.values():
        # 创建一个默认更新TTL的缓存实例
        default_cache = RedisFuncCache(
            __name__,
            cache._policy_type,
            client=redis_factory,
            maxsize=cache.maxsize,
        ...
        default_cache.policy.purge()

        # 但在装饰器中设置update_ttl=False
        @default_cache(update_ttl=False)
        def echo(x):
            return x
        ...
        # 第二次调用，应该命中缓存但不更新TTL
        with patch.object(default_cache, "put") as mock_put:
            result2 = echo(val)
            assert result2 == val
            # 在update_ttl=False模式下，缓存命中不应该触发重新存储
            mock_put.assert_not_called()
...
def test_update_ttl_override_true_in_decorate():
    """测试在decorate方法中覆盖update_ttl参数为True（实例级别为False）"""
    for cache in CACHES.values():
        # 创建一个默认不更新TTL的缓存实例
        no_update_ttl_cache = RedisFuncCache(
            __name__,
            ...
            maxsize=cache.maxsize,
            ttl=2,  # 2秒TTL
            update_ttl=False,  # 实例级别不更新TTL
        )
        no_update_ttl_cache.policy.purge()

        # 但在装饰器中设置update_ttl=True
        @no_update_ttl_cache(update_ttl=True)
        def echo(x):
            return x
        ...
        # 第二次调用，应该命中缓存并更新TTL（因为装饰器中覆盖为True）
        with patch.object(no_update_ttl_cache, "put") as mock_put:
            result2 = echo(val)
            assert result2 == val
            # 在update_ttl=True模式下，缓存命中不应该触发重新存储
